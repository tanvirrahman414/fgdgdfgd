...
// 🛑 FIXED: Renamed to clientLoader and removed 'fast-glob'
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  // We return an empty list because we cannot scan files in the browser.
  // This allows the build to finish successfully for cPanel.
  return {
    path: `/${params['*']}`,
    pages: [], 
  };
...
export default function CreateDefaultNotFoundPage({
  ...
  // 🛑 FIXED: Updated type to match clientLoader
  ...
  // Safe default if pages is undefined
  const pages = loaderData.pages || [];
  const missingPath = loaderData.path.replace(/^\//, '');
  
  const existingRoutes = pages.map((page: any) => ({
    path: page.path,
    url: page.url,
  }));
